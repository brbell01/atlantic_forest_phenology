...
for (i in 1:length(S1_combined)) {
  ...
  filename_base <- dir[[i]] %>% substring(1,nchar(dir[[i]])-4)
  summary <- c("R2"=R2, "MAPE"=Mape) %>% data.frame()
  names(summary) <- as.vector(dir[[i]])
...
# year-by-year
years <- S1_combined[[1]]$Date %>% as.Date(S1_combined[[i]]$Date, format = "%b %d, %Y") %>% year() %>% unique()
summary_table <- vector(mode = "list", length = length(S1_combined))
...
for (i in 1:length(S1_combined)) {
  ...
  names(S1_combined[[i]])[1] <- "Date"
  S1_combined[[i]]$Date <- as.Date(S1_combined[[i]]$Date, format = "%b %d, %Y")
  for (year in years){ # change this loop to iterate over years
    yeardata <- S1_combined[[i]] %>% filter(year(Date) == year)
    ...
    # Write out processed file
    filename_base <- substring(names(S1_combined[i]), 1, nchar(names(S1_combined[i])) - 4)
...
write.csv(alltable, paste0(wd, season_directory, "S1_", S1_band, "_timesat_model_fit_stats.csv"))

#write out summary file
write.csv(S1_combined, paste0(wd, season_directory, "S1", "_timesat_model_combined_data.csv"))

#write out annual data files
write.csv(summary_table[[1]], paste0(wd, "/mata_escura_S1_", S1_band, "_timesat_model_annual_fit_stats.csv"))
write.csv(summary_table[[2]], paste0(wd, "/monte_pascoal_S1_", S1_band, "_timesat_model_annual_fit_stats.csv"))
write.csv(summary_table[[3]], paste0(wd, "/rio_doce_S1_", S1_band, "_timesat_model_annual_fit_stats.csv"))
write.csv(summary_table[[4]], paste0(wd, "/sooretama_S1_", S1_band, "_timesat_model_annual_fit_stats.csv"))

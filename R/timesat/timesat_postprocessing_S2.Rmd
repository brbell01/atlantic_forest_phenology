...
This script performs a series of post-processing steps on the outputs of the Timesat software package. (1) Gather raw and fitted time series S2 veg. index data and perform fit statistics calculations. (2) 
...
Read season data from Timesat output seasonality file and write out formatted seasonality file
```{r}
...
for (i in 1:length(S2_season)) {
  ...
  S2_season[[i]] <- S2_season[[i]] %>% mutate(season_half_date = season_half_length + peak_date)
  ...
  S2_season[[i]] <- S2_season[[i]] %>% mutate("season_ensemble_date" = season_ensemble_length + peak_date)

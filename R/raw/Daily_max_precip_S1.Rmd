...
studyarea <-  "REBIO Sooretama, ES" # "PE Rio Doce, MG" # "REBIO Sooretama, ES" #"PNH Monte Pascoal, BA" #  "REBIO Mata Escura, MG" 
studyarea_base <- "sooretama" # "monte_pascoal" rio_doce ; monte_pascoal ; sooretama
satellite <- "S1"
rainfall_name <- "IMERG"
rainfall_filename <- "Sooretama_IMERG_2016-2022.csv"  ;
...
raintable <- raintable %>% mutate(date = date_parse(raintable$system.time_start, format = "%b %d, %Y")) %>%                          select(-system.time_start) %>% relocate(date) %>% pad
...
bandvals <- tablemeans[, grep(band, names(tablemeans))] %>% unlist()
#bandvals <- bandvals %>% remove_outliers
...
rainfall <- cbind(raintable$date, pscaled) %>% as.data.frame()
rainfall$date <- ymd(as.Date(rainfall$V1, origin = "1970-01-01"))
rainfall <- rainfall %>% relocate(date) %>% select(-c(V1))
...
Filter rainfall to days for S1 data exists
```{r}
plotdata <- s1 %>% left_join(rainfall, by = "date")
```
...
ggplot(data = plotdata) +
  geom_line(aes(x=date, y = s1[[2]] %>% remove_outliers), linewidth=0.45, color = "grey40") +
  geom_point(aes(x=date, y = s1[[2]] %>% remove_outliers), size=0.6) +
  geom_point(aes(x=date, y = pscaled), color="blue", alpha = 0.6, size=0.9) +
  #geom_ribbon(aes(x=date, y = p_rescaled, ymin = 0.6, ymax = 1), fill = 'blue', alpha=0.3) +
  #geom_ribbon(aes(ymin= ylims[1], ymax = ylims[2]), fill= "blue", alpha = 0.5, position = "identity") +

...
tables <- dir(wd, full.names = T, pattern = "VH_hectareID_") %>% map(read.csv, header=F)
for (i in 1:length(tables)) {
  names(tables[[i]])[1] <- "Date"
  names(tables[[i]])[2] <- "Median_VH"
  tables[[i]]$Date <- ymd(tables[[i]]$Date)
}

tables <- tables  %>% reduce(full_join, by = "Date") 

colnames <- list.files(pattern = "VH_hectareID_") %>%
...
tables.stats <- tables  %>%
  rowwise() %>% 
  mutate(medianVH_all = median(c_across(starts_with("hectareID")), na.rm = TRUE)) %>%
  rowwise() %>% 
  mutate(iqrVH_all = IQR(c_across(starts_with("hectareID")), na.rm = TRUE)) %>% 
  select(Date, medianVH_all, iqrVH_all)

latlon <- read.csv('hectares_latlon.csv', header = TRUE, sep = ',', colClasses = "character") 

#Plot time series data
#y-limits for plotting 
ylims <- range(tables.stats$medianVH_all)

ggplot(data=tables.stats, aes(x = Date, y = medianVH_all)) +  
  geom_line(color = "purple") +
  geom_ribbon(aes(ymin = medianVH_all - iqrVH_all, ymax = medianVH_all + iqrVH_all)
              , alpha=0.2) +
  ...
  ggtitle(paste0("Median (+ IQR) Bimonthly VH backscatter (dB) \n for ", length(tables)-1, 

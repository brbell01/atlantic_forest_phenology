...
tables <- dir(wd, full.names = T, pattern = "medianEVI_hectareID_") %>% map(read.csv, header=F)
for (i in 1:length(tables)) {
  names(tables[[i]])[1] <- "Date"
  names(tables[[i]])[2] <- "Median_EVI_SR"
  tables[[i]]$Date <- ymd(tables[[i]]$Date)
}

tables <- tables  %>% reduce(full_join, by = "Date") 
       
colnames <- list.files(pattern = "medianEVI_hectareID_") %>%
...
#       ,vjust = -0.6, hjust = -1.1)
  ...
  scale_y_continuous("Median EVI, Sentinel-2 (L2A)", limits = c(0, 1)
                     , breaks = seq(0,1,0.1)) + 
  ggtitle(paste0("Median (+ IQR) Bimonthly EVI Surface Reflectances Values \n for ", length(tables)-1, " 1-ha plots over Seasonal Semi-Deciduous Forest, Brazil"))
